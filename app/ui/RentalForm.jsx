...
const RentalForm = () => {
  ...
  const validateStep = (step) => {
    switch (step) {
      case 1:
        const dateError = formData.date.trim() === "" ? "La fecha es obligatoria" : "";
        setErrors({ date: dateError });
  
        return dateError === "";
        
      case 2:
        const timeError = formData.time.trim() === "" ? "La hora es obligatoria" : "";
          ...
          <div className="fixed top-0 left-0 h-full w-full bg-black/40 z-[1000] flex items-center justify-center">
            ...
            <div className="bg-white max-h-[90vh] w-[500px] rounded-[10px] scrollbar-none">
              ...
              <div className="bg-[#F3F6F9] p-2 rounded-[10px] flex justify-between items-center">
                ...
                {
                  (setps && (setps<4)) && (
                    <CircularProgressExample step={setps}></CircularProgressExample>
                  )
                }
                  
                  <span className="text-xl font-medium">{modalOptions[setps].title}</span>
                </div>

                <Image onClick={()=>{
                  setModal(false)
                }} className="cursor-pointer mr-4" src={"/icons/x.png"} width={25} height={25}></Image>
              ...
              <div>
                {setps === 1 && <FirstStep formData={formData} errors={errors} setFormData={setFormData} onNext={() => setSetps(2)} />}
                {setps === 2 && <SecondStep formData={formData} errors={errors} setFormData={setFormData} onNext={() => setSetps(3)} />}
                {setps === 3 && <ThirdStep formData={formData} errors={errors} setFormData={setFormData} onNext={() => setSetps(4)} />}
